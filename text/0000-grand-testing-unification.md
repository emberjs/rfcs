...
- Should we provide an API that will be invoked on all addons, so that they can automatically register custom helpers, waiters, assertions, etc? This could work similarly to `ember-load-initializers`, and auto-register files found in addons that match a given pattern (`addon-test-support/helpers`, `addon-test-support/waiters`, etc)...
