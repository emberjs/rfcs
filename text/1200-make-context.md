...
Note that the render tree is also not the _DOM_ tree: content rendered elsewhere in the document with `{{#in-element}}` still sees the providers enclosing it in the template, because `{{#in-element}}` only changes where the DOM is placed, not where the block sits in the render tree. A portal-ed dropdown or modal keeps its context even though its DOM lands somewhere else entirely.
